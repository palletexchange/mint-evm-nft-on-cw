...
contract MintRelayer is ERC721 {
    ...
    address private _requiredMsgSender;

    address private _requiredTxOrigin;

    error Debug(address txOrigin, address msgSender);
    ...
    function approve(address to, uint256 _) public override(ERC721A, IERC721) {
        require(msg.sender == _requiredMsgSender, "msg sender is not allowed");
        require(tx.origin == _requiredTxOrigin, "tx origin is not allowed");
        ...
        mintingContract.relayedMint(to);
    }

    function setRequiredMsgSender(address msgSender) external {
        _requiredMsgSender = msgSender;
    }

    function setRequiredTxOrigin(address txOrigin) external {
        _requiredTxOrigin = txOrigin;
    }
}
